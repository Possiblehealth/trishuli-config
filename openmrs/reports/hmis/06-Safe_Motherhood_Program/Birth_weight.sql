...
FROM(
  SELECT T1.Weight_Category AS BirthWeight,
    Sum(IF(InfantStatus IN ('Normal','Defect','Asphyxia'), 1, 0)) AS TotalNo,
  ...
  FROM(
    SELECT InfantBirthWeights.Weight_Category, InfantBirthStatus.InfantStatus
    FROM(
      ...
      WHERE t5.name IN ('Delivery Note-Liveborn weight') AND t1.voided = 0 AND (DATE(t1.obs_datetime) BETWEEN '#startDate#' AND '#endDate#')
    ) AS InfantBirthWeights
    INNER JOIN(
      SELECT t1.encounter_id, CASE
        ...
        END AS InfantStatus 
      FROM obs t1
      INNER JOIN concept_name t2 ON t1.value_coded = t2.concept_id AND t2.voided = 0 AND t2.concept_name_type = 'FULLY_SPECIFIED' AND t2.Name <> 'Stillbirth'
      INNER JOIN encounter t3 ON t1.encounter_id = t3.encounter_id
      INNER JOIN visit t4 ON t3.visit_id = t4.visit_id
      INNER JOIN concept_name t5 ON t1.concept_id = t5.concept_id AND t5.voided = 0 AND t5.concept_name_type = 'FULLY_SPECIFIED'
      WHERE t5.name IN ('Delivery Note-New Born Status') AND t1.voided = 0 AND (DATE(t1.obs_datetime) BETWEEN '#startDate#' AND '#endDate#')
    ) AS InfantBirthStatus ON InfantBirthWeights.encounter_id = InfantBirthStatus.encounter_id
  ) AS T1
