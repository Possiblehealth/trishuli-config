...
FROM
    ...
    LEFT  JOIN (SELECT DISTINCT value_reference AS type FROM visit_attribute) visit_type on visit_type.type in ('OPD','IPD')
    LEFT JOIN(
    SELECT distinct pi.identifier AS ip, p.gender as gender, visit_attribute.value_reference as visit_type
    FROM obs o
    ...
    WHERE(o.value_coded IS NOT NULL) AND DATE(e.encounter_datetime) BETWEEN DATE('#startDate#') AND DATE('#endDate#') group by pi.identifier) a ON a.gender = gender.gender AND a.visit_type =  visit_type.type
