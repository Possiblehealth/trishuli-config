        ...
        LEFT outer JOIN (SELECT 
            ...
            FROM(SELECT 
                ...
                CASE
                  ...
                  WHEN TIMESTAMPDIFF(YEAR, p.birthdate, o.value_datetime) > 19 AND TIMESTAMPDIFF(YEAR, p.birthdate, o.value_datetime) < 35 THEN '20 - 34 years'
                  WHEN TIMESTAMPDIFF(YEAR, p.birthdate, o.value_datetime) > 34 THEN '> 34 years'
                END AS agegroup
    FROM obs o
      INNER JOIN concept_name cn1 ON o.concept_id = cn1.concept_id AND cn1.concept_name_type = 'FULLY_SPECIFIED' AND cn1.name = 'Delivery Note-Delivery date and time' AND o.voided = 0 AND cn1.voided = 0
      ...
      INNER JOIN person p ON o.person_id = p.person_id AND p.voided = 0
        ...
        AND gestation_period_obs.concept_id = (
          SELECT concept_id
          FROM concept_view
          WHERE concept_full_name = 'Delivery Note-Gestation period') AND gestation_period_obs.voided = FALSE
      WHERE(o.value_datetime IS NOT NULL) AND DATE(e.encounter_datetime) BETWEEN DATE('#startDate#') AND DATE('#endDate#') group by ip) a
